    ...
    <div className="flex flex-col h-full bg-[#0a0a0a]">
      ...
      <div className="sticky top-0 z-30 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5 px-4 md:px-8 py-4 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={exportExcel} className="h-[44px] px-4 rounded-xl bg-white/5 border border-white/10 text-zinc-400 hover:text-white transition-all flex items-center gap-2">
            <Download className="w-4 h-4" />
            <span className="hidden md:inline text-[10px] font-black uppercase tracking-widest">Excel</span>
          </button>
          <button onClick={exportPDF} className="h-[44px] px-6 bg-white text-black font-black uppercase tracking-widest rounded-xl text-[10px] hover:bg-zinc-200 transition-all flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Main Chart Card */}
          <div className="lg:col-span-7 bg-[#141414] border border-white/5 rounded-[2.5rem] p-8 relative overflow-hidden">
            ...
            <div className="h-[400px] w-full">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none mt-4">
                <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em] mb-1">Impact</p>
                <p className="text-4xl font-black text-white tracking-tighter">{activeData.length}</p>
                <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Slices</p>
              </div>
            </div>
          </div>

          {/* Sorted Summary Table */}
          <div className="lg:col-span-5 bg-[#141414] border border-white/5 rounded-[2.5rem] p-8">
            <h3 className="text-sm font-black text-white uppercase tracking-widest mb-6">Efficiency Rankings</h3>
            <div className="space-y-4">
              {activeData.map((d: any, i: number) => (
                <div 
                  key={d.name}
                  className={cn(
                    "p-4 rounded-2xl border border-white/5 flex items-center justify-between group transition-all",
                    i === 0 ? "bg-blue-500/5 border-blue-500/10" : "bg-white/[0.02]"
                  )}
                >
                  <div className="flex items-center gap-4">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white/5 text-[10px] font-black text-zinc-500">
        ...
        <div className="bg-[#141414] border border-white/5 rounded-[2.5rem] overflow-hidden">
          <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-widest">Audit Ledger</h3>
              <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest mt-1">Chronological Sequence</p>
            </div>
            <div className="flex items-center gap-4">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="px-8 py-4 text-[9px] font-black text-zinc-600 uppercase tracking-widest">S.No</th>
                  <th className="px-6 py-4 text-[9px] font-black text-zinc-600 uppercase tracking-widest">Date</th>
                  <th className="px-6 py-4 text-[9px] font-black text-zinc-600 uppercase tracking-widest">Category</th>
                  <th className="px-6 py-4 text-[9px] font-black text-zinc-600 uppercase tracking-widest">Notes</th>
                  <th className="px-8 py-4 text-[9px] font-black text-zinc-600 uppercase tracking-widest text-right">Amount</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/[0.02]">
                {data.transactions.slice(0, 15).map((t: any, i: number) => (
                  <tr key={t.id} className="hover:bg-white/[0.02] transition-colors group">
                    <td className="px-8 py-4 text-[10px] font-black text-zinc-600">{String(i + 1).padStart(2, '0')}</td>
                    <td className="px-6 py-4 text-[11px] font-bold text-zinc-400">{new Date(t.date).toLocaleDateString('en-GB')}</td>
                    <td className="px-6 py-4">
                      <span className="text-[10px] font-black text-white px-2 py-1 bg-white/5 rounded-lg border border-white/5">
                        {t.categories?.name}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-[11px] text-zinc-500 max-w-[300px] truncate">{t.description || '—'}</td>
